		...
		switch (menu_0_focus)
		{
			...
			case 1:
				...
				if (credit_exchange_lerp_progress > 0.7)
				{
					...
					for (var _i = 0; _i < 12; _i++)
					{
						...
						{
							var _sprite = asset_get_index("spr_card_" + string(floor(_i / 4) + 101 + ((credit_exchange_card_start_row - 1))));
						}
						else
						{
							var _sprite = asset_get_index("spr_card_mystery");
			...
			case 2:
				...
				if (booster_menu_lerp_progress > 0.7)
				{
					...
					draw_sprite(spr_booster_packs, 1, NATIVE_RESOLUTION_WIDTH * 0.25 - 59, 120);
					scr_text_setup(global.font_large, fa_center, fa_top, c_white);
					draw_text(NATIVE_RESOLUTION_WIDTH * 0.25, 205, "Price: 20");

					draw_sprite(spr_booster_packs, 2, NATIVE_RESOLUTION_WIDTH * 0.5 - 59, 120);
					scr_text_setup(global.font_large, fa_center, fa_top, c_white);
					draw_text(NATIVE_RESOLUTION_WIDTH * 0.5, 205, "Price: 40");

					draw_sprite(spr_booster_packs, 0, NATIVE_RESOLUTION_WIDTH * 0.75 - 59, 120);
					scr_text_setup(global.font_large, fa_center, fa_top, c_white);
					draw_text(NATIVE_RESOLUTION_WIDTH * 0.75, 205, "Price: 10");
			...
			case 3:
				...
				if (quest_menu_lerp_progress > 0.7)
				{
					...
					{
						if ((quest_option_focus == 0) || ((quest_option_focus > 0) && (quest_status[quest_option_focus - 1] == 1)))
						{
							scr_text_setup(global.font_large, fa_left, fa_top, c_white);
							...
							draw_text(quest_detail_start_x + 16 + (CARD_WIDTH), quest_detail_start_y + 105 + (CARD_HEIGHT * 0.5), "x 3");
						...
						else
						{
							scr_text_setup(global.font_large, fa_left, fa_top, c_white);
							draw_text(quest_detail_start_x + 8, quest_detail_start_y + 5, "Milestone Details:")
							draw_text_ext(quest_detail_start_x + 8, quest_detail_start_y + 25, "The council thinks you're not ready for this yet, please check back once you have completed the previous milestone.", 16, quest_detail_width - 16);
					
							draw_text(quest_detail_start_x + 8, quest_detail_start_y + 85, "Require:")
							draw_sprite(spr_card_mystery, 0, quest_detail_start_x + 8 + (CARD_WIDTH * 0.5), quest_detail_start_y + 105 +CARD_HEIGHT);
							scr_text_setup(global.font_large, fa_left, fa_middle, c_white);
							draw_text(quest_detail_start_x + 16 + (CARD_WIDTH), quest_detail_start_y + 105 + (CARD_HEIGHT * 0.5), "x ?");
						}
					}
				}
...
{
	scr_text_setup(global.font_large, fa_center, fa_middle, c_white);
	draw_text(NATIVE_GUI_RESOLUTION_WIDTH * 0.5, 100, "You have completed all the milestones in Project Gaia in " + string(global.game_day) + " days!");
	scr_text_setup(global.font_small, fa_center, fa_middle, c_white);
	draw_text(NATIVE_GUI_RESOLUTION_WIDTH * 0.5, 140, "Card tech development has greatly progressed due to your contribution, and with this, humanity moves another step forward.");
	draw_text(NATIVE_GUI_RESOLUTION_WIDTH * 0.5, 170, "You can continue to experiment with various cards and discover various things, but remember that Project Gaia is forever thankful.");
	draw_text(NATIVE_GUI_RESOLUTION_WIDTH * 0.5, 200, "You can also participate in the project from the beginning once more, perhaps you could accomplist the same feat even faster now!");
	
	draw_text(NATIVE_GUI_RESOLUTION_WIDTH * 0.5, 250, "- Press any key to continue your game on a new day. -");
